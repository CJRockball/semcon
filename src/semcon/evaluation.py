...
def tune_threshold(y_true, scores, criterion: str = 'mcc') -> float:
    ...
    logger.info(f'Threshold tuned on OOF ({criterion}): {thr[i]:.4f} '
...
def operating_points(y_true, scores,
                     recall_targets=(0.5, 0.6, 0.7)) -> pd.DataFrame:
    ...
    for rt in recall_targets:
        ...
        rows.append({'recall_target': rt, 'threshold': float(thr[i]),
...
def save_confusion_heatmap(y_true, scores, threshold: float,
                           out: Path, title: str = 'Holdout') -> None:
    ...
    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', cbar=False,
                ...
                yticklabels=['true pass', 'true fail'], ax=ax)
