...
def compute_limits(train: pd.DataFrame, method: str = "robust",
    ...
    for col in train.columns:
        ...
        if len(x) < 25:
            rows[col] = dict(center=np.nan, sigma=np.nan, lcl=np.nan, ucl=np.nan,
                             mr_bar=mr_bar, mr_ucl=np.nan, degenerate=True)
            continue
        if method == "robust":
        ...
        rows[col] = dict(center=center, sigma=sigma,
                         lcl=center - k * sigma, ucl=center + k * sigma,
                         mr_bar=mr_bar, mr_ucl=float(D4 * mr_bar),
...
def ewma(x: pd.Series, lam: float, center: float, sigma: float,
         k: float = 3.0) -> tuple[pd.Series, float, float]:
...
def screening(df: pd.DataFrame, cols: list[str], limits: pd.DataFrame,
              i_hold: int, i_tail: int, ewma_lam: float = 0.2) -> pd.DataFrame:
    """One row per sensor: frozen-limit alarm rates per phase + missingness rates."""
    phases = {"p1": slice(0, i_hold),
              "p2": slice(i_hold, len(df)),
              "tail": slice(i_tail, len(df))}
    rows = {}
    for col in cols:
        lim = limits.loc[col]
...
def windowed_rate(s: pd.Series, window: int) -> pd.DataFrame:
    ...
    return pd.DataFrame({"rate": grp.mean(), "n": grp.size(),
                         "x": grp.apply(lambda v: v.index[-1]).astype(int)})
    ...
    try:
        if p.suffix == ".json":
            ...
            t = (pd.read_parquet(p) if p.suffix == ".parquet"
                 else pd.read_csv(p, index_col=0))
...
def select_showcase(screen: pd.DataFrame, stable: set[str], delta_min: float,
                    n_showcase: int) -> tuple[list[str], dict]:
...
def load_master_notes(path: str | None) -> dict[str, str]:
    ...
    for f in m.index:
        bits = []
        for colname, fmt in [("hedgesg", "g={:.2f}"), ("log2vrclip", "log2VR={:.1f}"),
...
def plot_overview(screen: pd.DataFrame, stable: set[str], out_png: Path) -> None:
    ...
    ax.scatter(s.loc[~inst, "ooc_p1"] + eps, s.loc[~inst, "ooc_p2"] + eps,
               s=12, c="0.65", label="screened sensors")
    ax.scatter(s.loc[inst, "ooc_p1"] + eps, s.loc[inst, "ooc_p2"] + eps,
               s=26, c="tab:red", label="model-stable set")
    ax.plot([eps, 1], [eps, 1], ls="--", c="k", lw=1)
    for name, r in s.nlargest(3, "delta").iterrows():
        ax.annotate(name, (r["ooc_p1"] + eps, r["ooc_p2"] + eps), fontsize=8,
...
def plot_imr(df: pd.DataFrame, col: str, lim: pd.Series, i_hold: int, i_tail: int,
             lam: float, note: str, out_png: Path) -> None:
    x = df[col]
    t = np.arange(len(df))
    ...
    ax.plot(t[fails & obs], xv[fails & obs], "x", ms=7, color="red",
            label="fail wafer")
    extra = (wr[["r2", "r3", "r4"]].any(axis=1).to_numpy()
             & ~wr["r1"].to_numpy() & obs)
    ax.plot(t[extra], xv[extra], "o", mfc="none", mec="orange", ms=9,
            label="WE rule 2-4")
    for y, c, lb in [(lim["ucl"], "r", f'UCL {lim["ucl"]:.3g}'),
    ...
    axes[1].axhline(lim["mr_ucl"], color="r", ls="--", lw=1,
                    label=f'MR UCL {lim["mr_ucl"]:.3g}')
...
def plot_protocol(df: pd.DataFrame, i_hold: int, i_tail: int, window: int,
                  out_dir: Path) -> None:
    ...
    for ax, c in zip(axes, RATE_FEATURES):
        ...
        ax.plot(w["x"].to_numpy()[beyond], w["rate"].to_numpy()[beyond],
                "o", mfc="none", mec="red", ms=9)
        for b in (i_hold, i_tail):
            ax.axvline(b, color="k", ls=":", lw=1)
        ax.set_title(f"{c} rate per {window} wafers (phase-I rate={p0:.3f})",
                     fontsize=10)
...
def plot_pchart(df: pd.DataFrame, i_hold: int, i_tail: int, window: int,
                out_png: Path) -> None:
    ...
    fig, ax = plt.subplots(figsize=(11, 4))
    ax.plot(w["x"], w["rate"], ".-", ms=4, lw=0.8,
            label=f"fail rate / {window} wafers")
    ...
    beyond = w["rate"].to_numpy() > ucl
    ax.plot(w["x"].to_numpy()[beyond], w["rate"].to_numpy()[beyond],
            "o", mfc="none", mec="red", ms=9, label="beyond UCL")
    for b in (i_hold, i_tail):
        ax.axvline(b, color="k", ls=":", lw=1)
    ax.set_title("Yield p-chart - LCL clips at 0, so improvement (the 0/27 tail) "
...
def main(argv=None):
    ...
    logger.info(f"screened {len(screen)} sensors "
    ...
    for col in picks:
        plot_imr(df, col, limits.loc[col], i_hold, i_tail,
                 lam=args.ewma_lam, note=notes.get(col, ""),
    ...
    tracking.append_index(run, {
        "type": "spc", "limits": args.limits, "note": args.note or "",
