...
def windowed_mean(df: pd.DataFrame, cols: list[str], start: int, end: int,
...
def daily_volume(df: pd.DataFrame, i_hold: int) -> pd.DataFrame:
    ...
    daily = (ph1.groupby(pd.Grouper(key="timestamp", freq="D"))
                .agg(total_fail=("target", "sum"),
                     fail_freq=("target", "mean"),
                     sample_size=("timestamp", "count"))
                .reset_index())
    logger.info(f"daily volume: {len(daily)} days, "
...
def save_forecast_fig(train: pd.Series, actual: pd.Series, fc_mean, fc_lo, fc_hi,
    ...
    ax.fill_between(fc_x, fc_lo, fc_hi, color="tab:red", alpha=0.15,
                    label="95% interval")
    ax.axvline(len(train) - 0.5, color="k", ls=":", lw=1)
    breach = (actual < fc_lo) | (actual > fc_hi)
    if breach.any():
        ax.plot(fc_x[breach], actual[breach], "o", mfc="none", mec="red",
                ms=12, label="breach")
    ax.set_title(title)
...
def fit_orders(series: pd.Series, orders: list, label: str) -> tuple[pd.DataFrame, dict]:
    ...
    for order in orders:
        try:
            ...
            continue
        models[order] = res
        table.append({"order": order, "aic": res.aic, "aicc": res.aicc,
                      "bic": res.bic})
...
def rolling_origin(y: pd.Series, candidates: dict, min_train: int = 16,
    ...
    for t in range(min_train, len(y) - horizon + 1, step):
        fc = {"persistence": np.repeat(y[t - 1], horizon),
              "mean": np.repeat(y[:t].mean(), horizon)}
...
def forecast_holdout(train: pd.Series, hold: pd.Series, order, exog_tr,
    ...
    breach = save_forecast_fig(train, hold, mean, ci[:, 0], ci[:, 1],
    ...
    rows = [{"model": label,
            ...
            {"model": "persistence",
             ...
             "coverage": np.nan, "breaches": np.nan},
            {"model": "mean",
...
def seasonal_volume(df: pd.DataFrame, i_hold: int, figs, summaries,
    ...
    save_series_fig(daily["timestamp"], vol,
                    "wafers measured per day (phase I)",
                    figs / "volume_series.png")
    save_acf_pacf_fig(vol, lag, "daily volume", figs / "volume_acf_pacf.png")
    save_acf_pacf_fig(vol.diff(), lag, "daily volume, first difference",
                      figs / "volume_diff_acf_pacf.png")
    ...
    for name, (order, sorder) in SEASONAL_MODELS.items():
        try:
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                res = SARIMAX(vol, order=order, seasonal_order=sorder,
                              trend="c").fit()
        except Exception as e:
            logger.warning(f"{name} failed: {e}")
            continue
        models[name] = res
        rows.append({"model": name, "order": str(order),
                     "seasonal": str(sorder), "aic": res.aic,
                     "aicc": res.aicc, "bic": res.bic})
    ...
    save_acf_pacf_fig(models[best].resid, lag, f"residuals: {best}",
...
def exog_tests(frac: pd.Series, exog_df: pd.DataFrame, order) -> pd.DataFrame:
    ...
    rows = [{"exog": "(none)", "aicc": base.aicc, "coef": np.nan,
             "pvalue": np.nan, "sign_check": ""}]
...
def failrate_arima(df: pd.DataFrame, i_hold: int, i_tail: int, figs, summaries,
    ...
    save_series_fig(frac.index, frac,
    ...
    save_acf_pacf_fig(best.resid, lag, f"residuals: ARIMA{best_order}",
                      figs / "failfrac_resid_acf_pacf.png")

    # 2. staged exog tests on the frozen order (one extra parameter each)
    exog_p1 = windowed_mean(df, EXOG_CANDIDATES + EXOG_EXPLORATORY,
                            0, i_hold, window)
    ...
    logger.info(f"exog decision: {'use ' + exog_best['exog'] if use_exog else 'stay univariate'} "
    ...
    hold = forecast_holdout(frac, frac_hold, order_w, exog_w, exog_hold,
                            "failfrac", figs)
...
def main(argv=None):
    ...
    run, meta = tracking.make_run(
        ...
        run_name="sarimax", note=args.note or "")
    figs = run / "figures"
    ...
    fail = failrate_arima(df, i_hold, i_tail, figs, summaries,
                          window=args.window, min_train=args.min_train,
                          horizon=args.horizon)
    drift = channel_drift(df, i_hold, i_tail, figs, summaries,
                          window=args.window)

    tracking.append_index(run, {
        "type": "sarimax", "window": args.window, "note": args.note or "",
